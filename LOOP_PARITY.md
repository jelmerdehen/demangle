...
<!-- newest on top -->
- 2026-05-13 XD: When multiple syms produce identical wrap.Text (e.g. AttributedString.LocalizationValue.init 6 variants), dispatch on `p.s` source-mangling content (`strings.Contains(p.s, "12localization"/"17FormattingOptions")`) inside the post-emit fixup. +6 prod.
- 2026-05-13 XC: Foundation.LocalizedStringResource.init full-text replace covers parser-host-detection-wrong (renders as `.LocalizationValue.init` not `.init` directly). +2 prod.
